...
def train(limit: int | None = None, sft_start: int | None = None,
          ...
          traces_repo: str = "mateo0093/le-gros-chaton-traces"):
    os.chdir("/root/proj")
    # Reduce CUDA fragmentation (helped avoid OOMs on T4/L4 in testing)
    os.environ.setdefault("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")

    # For trajectory SFT: pull the traces generated by modal_gen_trajectories.py
    # (phase 2a) so the model trains on its own diverse, verified, self-reviewed
    # behavior — with the creativity + self-awareness baked in.
    token = os.environ.get("HF_TOKEN", "")
    if trajectory_sft and token:
        try:
            from huggingface_hub import hf_hub_download
            print(f"[modal] Pulling traces from {traces_repo}...")
            local = hf_hub_download(repo_id=traces_repo, repo_type="dataset",
                                    filename="agent_traces_full.jsonl", token=token)
            import shutil
            shutil.copy(local, "/root/proj/agent_traces_full.jsonl")
            n = sum(1 for _ in open("/root/proj/agent_traces_full.jsonl"))
            print(f"[modal] ✓ {n} traces ready")
        except Exception as e:
            print(f"[modal] Could not pull traces ({e}) — will use local file if present")
